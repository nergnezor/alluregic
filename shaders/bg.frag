...
const vec4 backgroundColor = vec4(0.1, 0.5, 0.8, 0.5);
const vec4 holeColor = vec4(0.3, 0.1, 0.0, 0.5);
const vec3 skinColor = vec3(0.9, 0.7, 0.6);

bool isHole(vec2 fragCoord, vec2 noseCenter, vec2 noseSideOffset, float noseSideRadius, vec3 skinColor, out vec4 fragColor)
{
    // Nose holes
    float holeToSideRatio = 0.5 + 0.3 * pow(sin(iTime), 2.0);
    float holeRadius = noseSideRadius * holeToSideRatio;
    vec2 holeOffsetR = vec2(noseSideOffset.x * holeToSideRatio, noseSideOffset.y / holeToSideRatio);
    vec2 holeOffsetL = vec2(-noseSideOffset.x * holeToSideRatio, noseSideOffset.y / holeToSideRatio);
    if (length(fragCoord - noseCenter - holeOffsetR) < holeRadius || length(fragCoord - noseCenter - holeOffsetL) < holeRadius) {
        fragColor = holeColor;
        return true;
    }
    return false;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Nose center
    if (distance < radius) {
        if (isHole(fragCoord, noseCenter, noseSideOffsetL, noseSideRadius, skinColor, fragColor)) {
            return;
        }
        fragColor = vec4(skinColor, 1.0);
        return;
    }
    bool isNoseSide = length(fragCoord - noseCenter - noseSideOffsetL) < noseSideRadius || length(fragCoord - noseCenter - noseSideOffsetR) < noseSideRadius;
    if (isNoseSide) {
        if (isHole(fragCoord, noseCenter, noseSideOffsetL, noseSideRadius, skinColor, fragColor)) {
            return;
        }
    ...
    if (!drawEyes(fragColor, fragCoord)) {
        fragColor = vec4(0.1, 0.5, 0.8, 0.5);
    }
