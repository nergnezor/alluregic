...
void mainImage(out vec4 O, vec2 u) {
  vec2 U = u + u - vec2(radius, radius);
  float T = 6.2832, l = length(U) / 0.5, L = ceil(l) * 6.,
        a = atan(U.x, U.y) - iTime * 4. * (fract(1e4 * sin(L)) - .5);
...
/* -------- SHAPERS/MISC -------- */
float fold(in float x) { return abs(mod(x + 1.0, 2.0) - 1.0); }
...
float camel_ramp(in float x, in float s) {
  float y = fract(x);
  return floor(x) +
         pow(0.5 - (0.5 * cos(6.28318530718 * y) * cos(3.14159265359 * y)), s);
}
...
float scale(in float x, in float inmin, in float inmax, in float outmin,
            in float outmax) {
...
vec4 mk_tearduct(in float sdf, in float t) {
  vec3 col = mix(vec3(0.0, 0.0, 0.0), vec3(0.8471, 0.8471, 0.8471),
                 cosine_ramp(fold(sdf * 60.0 + t * 0.25), 2.0));
  float a = smoothstep(edge, 0.0, sdf + edge);
  return vec4(col, a);
}

vec4 mk_eyelids(in float sdf) {
  return smoothstep(edge * 2.0, 0.0, abs(sdf) - 0.01) *
         vec4(0.6471, 0.6471, 0.6471, 1.0);
}
...
vec4 mk_sclera(in vec2 vUV, in float d, in float t) {
  ...
  vec4 sclera =
      smoothstep(edge, 0.0, d - 0.25) * vec4(0.8275, 0.8235, 0.8235, 1.0);
  vec4 border = smoothstep(edge, 0.0, abs(d - 0.25) - 0.0025) *
                vec4(0.2549, 0.2549, 0.2549, 1.0);
...
vec4 mk_iris(in vec2 vUV, in float d, in float t) {
  ...
  vec3 col = mix(vec3(0.6784, 0.7922, 0.8431), vec3(0.6118, 0.7255, 0.7804),
                 fold(cosine_ramp(sin(a * 3.0 * cos(a * 2.0) + t * 0.5), 4.0)));
  col =
      mix(col, vec3(0.7765, 0.8196, 0.8392),
          cosine_ramp(cos(3.0 * a * sin(-a * 1.5) + t * 0.4) * 0.5 + 0.5, 4.0));
  vec4 iris = smoothstep(edge, 0.0, d - 0.125) * vec4(col, 1.0);
  vec4 border = smoothstep(edge, 0.0, abs(d - 0.125) - 0.002) *
                vec4(0.2627, 0.2353, 0.2353, 1.0);
...
vec4 mk_pupil(in float d, in float t) {
  ...
  return smoothstep(edge, 0.0, d - 0.05 + t) *
         vec4(0.0627, 0.0588, 0.0588, 1.0);
}

vec4 mk_glow(in vec2 vUV, in float t) {
  float d = length(vUV);
  vUV *=
      vec2(sin(d * 2.123 - t * 0.798347), cos(d * 3.123 + t * 0.91823)) * 0.1 +
      1.0;
  ...
  d = length((vUV - (vUV.y * 0.1)) + 0.05);
  glow = mix(glow, smoothstep(edge * 1.25, 0.0, d - 0.02) * vec4(1.0),
             1.0 - glow.a);
...
void mainImage2(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec4 eye = mk_eye(vUV, 3 - pow(sin(iTime), 0.1), iTime * radius);
...
void main() {
  // vec4 fragColor;
  mainImage2(fragColor, FlutterFragCoord().xy);
